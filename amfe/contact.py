...
a return mapping algorithm, i.e. it requires the previous state. For now, this information is saved in the contact model.
...
class JenkinsContact(HysteresisContact):
    ...
    The contact is detected along the normal direction (z) and contact forces are computed with a linear penalty approach.
    ...
    def __init__(self, k_n=30,  k_t = 30, mu=0.3, u_rel_t= None, f_t = None):
        """
                ...
                k_t : float
                    Stiction/Spring stiffness in tangential direction. TODO: Potentially shape (2,) for non-isotropic friction
                mu : float
        ...
        if u_rel[2] < 0:
            ...
            if f_trial <= f_limit:
                ...
                if f_trial > 0:
                    ...
                    K[:2,:2] = self.k_t *f_limit/f_trial**3 * np.array([[trial_stress[1]**2, trial_stress[0]*trial_stress[1] ],
